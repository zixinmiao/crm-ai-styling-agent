...
- 飞书表（同一 Base `WKxvbOg1XaeYPRslkuVcpSoonic`）：
  - `商品知识库`（`tblSZwqFZZdA2rCp`）
  - `搭配知识库`（`tbl9WHzDBzOD8t6j`）
  - `门店库存库`（`tblVkg9lIp1DwOC8`）
  - `顾客画像记录`（`tbld1EbJRzlb0ogR`，按 `profile_id` 反查推荐约束）
  - `搭配候选记录`（`tblorQwKBBRJwPra`，写）
...
6. 最终输出 3-5 套候选，并标记：
   ...
   - `需替代款`
   - `库存不足`
7. 将结果写入「搭配候选记录」（table_id `tblorQwKBBRJwPra`），字段：`顾客画像ID` / `主单品款号` / `搭配内容` / `手机端卡片` / `导购话术`（可由 sales-script 后续补写）/ `试衣优先级` / `生成时间`。同时回传 `writeback_payload` 供下游引用。
